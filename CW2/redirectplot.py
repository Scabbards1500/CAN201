...
class forwardSwitch(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        self.latency_data.append(latency)  # 将延迟数据存储在列表中
        self.packet_count += 1
        print(self.packet_count)
        # 绘制曲线（每收集10个数据点绘制一次曲线）
        if self.packet_count % 5 == 0:
            self.plot_latency_curve()

        datapath.send_msg(out)

    def plot_latency_curve(self):
        plt.plot(self.latency_data)
        plt.axhline(np.mean(self.latency_data), color='red', linestyle='dashed')
        plt.xlabel('Packet Number')
        plt.ylabel('Latency (seconds)')
        plt.ylim(0, 0.004)
        plt.title('Network Latency Curve')
